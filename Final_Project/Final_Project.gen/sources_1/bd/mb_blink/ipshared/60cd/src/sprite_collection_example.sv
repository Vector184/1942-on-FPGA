...
logic signed [10:0] rel_x, under_rel_x;
logic [9:0] rel_y;
logic [9:0] rom_rel_x, rom_rel_y;
logic [9:0] under_rel_y;
...
logic sprite_sample_on, under_sprite_sample_on;
logic sprite_sample_on_q, under_sprite_sample_on_q;
logic sprite_on;
...
always_comb begin
	sprite_sample_on = 1'b0;
	if (selected_sprite_on &&
		(rel_x >= 11'sd0) &&
		(rel_x < $signed({5'b0, sprite_w})) &&
		(rel_y < {4'b0, sprite_h})) begin
		sprite_sample_on = 1'b1;
	end
end

always_comb begin
	under_sprite_sample_on = 1'b0;
	if (under_sprite_on &&
		(under_rel_x >= 11'sd0) &&
		(under_rel_x < $signed({5'b0, under_sprite_w})) &&
		(under_rel_y < {4'b0, under_sprite_h})) begin
		under_sprite_sample_on = 1'b1;
	end
end

always_comb begin
	if (attributes[6]) begin
		rom_rel_x = {4'b0, sprite_w} - 10'd1 - rel_x[9:0];
	end else begin
		rom_rel_x = rel_x[9:0];
	end
	...
	if (under_attributes[6]) begin
		under_rom_rel_x = {4'b0, under_sprite_w} - 10'd1 - under_rel_x[9:0];
	end else begin
		under_rom_rel_x = under_rel_x[9:0];
	end
...
always_ff @ (posedge negedge_vga_clk) begin
	sprite_sample_on_q <= sprite_sample_on;
	under_sprite_sample_on_q <= under_sprite_sample_on;
end

// address into the sprite sheet ROM
always_comb begin
	if (sprite_sample_on) begin
...
always_comb begin
	if (under_sprite_sample_on) begin
		under_rom_address = ((under_sheet_y + under_rom_rel_y) * 256 + (under_sheet_x + under_rom_rel_x));
	end else begin
		under_rom_address = 15'd0;
	end
end
assign top_pixel_visible = sprite_sample_on_q && ({palette_red, palette_green, palette_blue} != 12'hF5F);
assign under_pixel_visible = under_sprite_sample_on_q && ({under_palette_red, under_palette_green, under_palette_blue} != 12'hF5F);
