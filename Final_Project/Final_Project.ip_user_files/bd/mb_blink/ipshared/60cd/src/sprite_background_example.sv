module sprite_background_example (
	...
	input logic blank,
	output logic [11:0] scroll_y_status,
...
localparam DEATH_SCREEN_BASE_X = 0;
localparam DEATH_SCREEN_BASE_Y = 240;
localparam DEATH_SCREEN_W = 256;
localparam DEATH_SCREEN_H = 240;
...
localparam HUD_1UP_X = 42;
localparam HUD_1UP_Y = 26;
localparam HUD_1UP_W = 22;
localparam HUD_1UP_H = 7;
localparam HUD_2UP_X = 194;
localparam HUD_2UP_Y = 26;
localparam HUD_2UP_W = 22;
localparam HUD_2UP_H = 7;
localparam HUD_ROUND_X = 209;
localparam HUD_ROUND_Y = 209;
localparam HUD_ROUND_W = 14;
localparam HUD_ROUND_H = 8;
...
logic [6:0] hud_blink_count = 7'd0;
logic intro_overlay_last = 1'b0;
logic frame_pulse;
assign frame_pulse = (DrawX == 255) && (DrawY == 239);
assign scroll_y_status = scroll_y;

logic full_overlay_active;
logic hud_1up_on;
logic hud_2up_on;
logic hud_round_on;
logic hud_overlay_on;
logic overlay_pixel_visible;

assign full_overlay_active = game_regs[0][0] || game_regs[0][1] || game_regs[0][2] || game_regs[0][4] || game_regs[0][5];
assign hud_1up_on = !full_overlay_active && !game_regs[0][6] && !hud_blink_count[6] &&
                    (DrawX >= HUD_1UP_X) && (DrawX < HUD_1UP_X + HUD_1UP_W) &&
                    (DrawY >= HUD_1UP_Y) && (DrawY < HUD_1UP_Y + HUD_1UP_H);
assign hud_2up_on = !full_overlay_active && game_regs[0][6] && !hud_blink_count[6] &&
                    (DrawX >= HUD_2UP_X) && (DrawX < HUD_2UP_X + HUD_2UP_W) &&
                    (DrawY >= HUD_2UP_Y) && (DrawY < HUD_2UP_Y + HUD_2UP_H);
assign hud_round_on = !full_overlay_active &&
                      (DrawX >= HUD_ROUND_X) && (DrawX < HUD_ROUND_X + HUD_ROUND_W) &&
                      (DrawY >= HUD_ROUND_Y) && (DrawY < HUD_ROUND_Y + HUD_ROUND_H);
assign hud_overlay_on = hud_1up_on || hud_2up_on || hud_round_on;
assign overlay_pixel_visible = ({overlay_red, overlay_green, overlay_blue} != 12'hF5F);

always_ff @(posedge vga_clk or posedge reset) begin
    if (reset) begin
        scroll_y <= START_SCROLL_Y;
        frame_count <= 6'd0;
        hud_blink_count <= 7'd0;
        intro_overlay_last <= 1'b0;
    end else begin
        intro_overlay_last <= game_regs[0][0];
        if (game_regs[0][1]) begin
            scroll_y <= START_SCROLL_Y;
            frame_count <= 6'd0;
            hud_blink_count <= 7'd0;
        end else if (game_regs[0][0] && !intro_overlay_last) begin
            scroll_y <= START_SCROLL_Y;
            frame_count <= 6'd0;
        end else if (game_regs[0][3]) begin
            if (frame_pulse) begin
                hud_blink_count <= hud_blink_count + 1;
            end
...
always_comb begin
    if (game_regs[0][4] && DrawX < DEATH_SCREEN_W && DrawY < DEATH_SCREEN_H)
        overlay_rom_address = (DEATH_SCREEN_BASE_Y + DrawY) * 256 + (DEATH_SCREEN_BASE_X + DrawX);
    else if (game_regs[0][1] && DrawX < START_SCREEN_W && DrawY < START_SCREEN_H)
...
always_ff @ (posedge vga_clk) begin
	...
	if (blank) begin
		if (game_regs[0][5]) begin
			red <= 4'h0;
			green <= 4'h0;
			blue <= 4'h0;
		end else if (game_regs[0][4]) begin
			red <= overlay_red;
			green <= overlay_green;
			blue <= overlay_blue;
		end else if (game_regs[0][1]) begin
			red <= overlay_red;
			green <= overlay_green;
			blue <= overlay_blue;
		end else if (game_regs[0][2] && overlay_pixel_visible) begin
			red <= overlay_red;
			green <= overlay_green;
			blue <= overlay_blue;
		end else if (game_regs[0][0] && overlay_pixel_visible) begin
			red <= overlay_red;
			green <= overlay_green;
			blue <= overlay_blue;
		end else if (hud_overlay_on && overlay_pixel_visible) begin
			red <= overlay_red;
			green <= overlay_green;
			blue <= overlay_blue;
		end else begin
