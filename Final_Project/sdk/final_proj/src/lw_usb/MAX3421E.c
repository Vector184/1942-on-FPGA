...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE sendbuffer[2];
	sendbuffer[0] = reg  + 0x02;
	sendbuffer[1] = val;
	BYTE receivebuffer[2];
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	if (XSpi_Transfer(&SpiInstance, sendbuffer, receivebuffer, 2) != 0){
		xil_printf("\n Single-write SPI Error");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
		...
		XSpi_SetSlaveSelect(&SpiInstance, 0x01);
		if (XSpi_Transfer(&SpiInstance, sendbuffer, receivebuffer, nbytes + 1) != 0){
			xil_printf("\nMulti-write SPI Error");
		}
		XSpi_SetSlaveSelect(&SpiInstance, 0x00);
		return (data + nbytes);
}
...
void MAX3421E_init(void) {
	...
	XTmrCtr_SetOptions(&Usb_timer, 0, XTC_AUTO_RELOAD_OPTION);
	XTmrCtr_SetResetValue(&Usb_timer, 0, 0);
	XTmrCtr_Reset(&Usb_timer, 0);
