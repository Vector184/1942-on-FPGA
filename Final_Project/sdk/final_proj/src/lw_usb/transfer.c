...
{
 DWORD start_time = usb_ticks();
 DWORD timeout_ticks = (DWORD)USB_XFER_TIMEOUT * TICKS_PER_MILLIS;
 BYTE tmpdata;
 BYTE rcode;
 char retry_count = 0;
 BYTE nak_count = 0;
 BYTE nak_limit = USB_NAK_LIMIT;

 if ((token == tokIN) && (ep == 1)) {
     timeout_ticks = 2U * TICKS_PER_MILLIS;
     nak_limit = 1;
 }
    ...
    while( 1 ) {
        ...
        rcode = 0xff;
        while( (DWORD)(usb_ticks() - start_time) < timeout_ticks ) {
...
void USB_Task( void )
{
 static DWORD usb_delay_start = 0;
        ...
        case( USB_STATE_ATTACHED ):                     //prepare for enumeration
            switch( usb_task_state ) {
                case( USB_STATE_ATTACHED ):
                    usb_delay_start = usb_ticks();           //initial settle 200ms
                	usb_task_state = USB_ATTACHED_SUBSTATE_SETTLE;
                    break;//case( USB_STATE_ATTACHED )
                case( USB_ATTACHED_SUBSTATE_SETTLE ):   //waiting for settle timer to expire
                    if( (DWORD)(usb_ticks() - usb_delay_start) > ((DWORD)USB_SETTLE_TIME * TICKS_PER_MILLIS) ) {
