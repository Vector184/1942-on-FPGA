...
Sprite(id,frameNo,dstRect,point,isVisible,type,currFilm)
{
    ...
    setAnimationEnemyBulletFilm( AnimationFilmHolder::Get()->GetFilm("BULLET_ENEMY") );
...
void EnemyFighter::setRemainingBullets(unsigned BULLET_SUPERACE){
    this->remainingBullets = BULLET_SUPERACE;
}
...
void EnemyFighter::setFrame(unsigned i) {
    ...
    if(_enemyType != EnemyFighterType(RED_PLANE)){
        ...
        rect.h = _currFilm->getFrameBox(getFrame()).h * Game::getSpriteSize();
        rect.w = _currFilm->getFrameBox(getFrame()).w * Game::getSpriteSize();
        setDstRect(rect);
    }
     */
}
...
void EnemyFighter::fireStraightBullet(){
    ...
    AnimationFilm* fireAnimationFilm = AnimationFilmHolder::Get()->GetFilm("BULLET_ENEMY");
...
void EnemyFighter::createPowerUp(){
    ...
    AnimationFilm* powerUpAnimationFilm = AnimationFilmHolder::Get()->GetFilm("POWER_UP");
    Sprite* powerUp;
