...
#if defined(__GNUC__)
#define USB_ISR_CODE __attribute__((section(".usb_isr"), noinline))
#else
#define USB_ISR_CODE
#endif
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE sendBuffer[2] = {reg+2, val};
	BYTE receiveBuffer[2];
	if(XSpi_Transfer(&SpiInstance, sendBuffer, receiveBuffer, 2) != XST_SUCCESS){
		xil_printf("ERROR: SPI device fail to write single byte\n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	BYTE sendBuffer[1+nbytes];
	BYTE receiveBuffer[1+nbytes];
	sendBuffer[0] = reg+2;
	for(int i=0 ; i<nbytes ; i++){
		sendBuffer[i+1] = data[i];
	}
	if(XSpi_Transfer(&SpiInstance, sendBuffer, receiveBuffer, nbytes+1) != XST_SUCCESS){
		xil_printf("ERROR: SPI device fail to write multiple bytes\n");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x0);
	return (data+nbytes);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return val
	BYTE sendBuffer[2] = {reg, 0};
	BYTE receiveBuffer[2];
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	if(XSpi_Transfer(&SpiInstance, sendBuffer, receiveBuffer, 2) != XST_SUCCESS){
		xil_printf("ERROR: SPI device fail to read single byte\n");
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if(XSpi_Transfer(&SpiInstance, sendBuffer, receiveBuffer, nbytes+1) != XST_SUCCESS){
		xil_printf("ERROR: SPI device fail to read multiple bytes\n");
	}
	for(int i=0 ; i<nbytes ; i++){
...
USB_ISR_CODE void MAX3421E_Task(void) {
...
USB_ISR_CODE void MaxIntHandler(void) {
	BYTE HIRQ;
