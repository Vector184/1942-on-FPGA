...
void AnimatorHolder::createExplosion(SDL_Rect dstRect){
    ...
    AnimationFilm* fireAnimationFilm = AnimationFilmHolder::Get()->GetFilm("EXPLOSION_ENEMY");
    
    SDL_Rect rect = { dstRect.x+dstRect.w/2-10 , dstRect.y+dstRect.h/2-10 , 20 , 20 };
    
    Sprite* EXPLOSION_ENEMY = new Sprite(spriteExplosionId, 0, rect , {0,0}, true, POWER_UPS, fireAnimationFilm);
    assert(EXPLOSION_ENEMY);
    
    SoundHolder::playSound("EXPLOSION_ENEMY");
    
    Animation* EXPLOSION_ENEMYAnimation = AnimationHolder::getAnimationHolder()->getAnimation("EXPLOSION_ENEMY");
    assert(EXPLOSION_ENEMYAnimation);
    
    MovingPathAnimator* EXPLOSION_ENEMYAnimator = new MovingPathAnimator(animatorExplosionId, EXPLOSION_ENEMY, (MovingPathAnimation*)EXPLOSION_ENEMYAnimation);
    assert(EXPLOSION_ENEMYAnimator);
    
    EXPLOSION_ENEMYAnimator->start(Game::getGameTime());
...
void AnimatorHolder::triggerstartEnemyPlanesTickAnimator(){
	...
	if (SoundHolder::playMusic() == -1){
		...
		printf("Music playback unavailable on FPGA build\n");
#else
    ...
    //Bullets
    TimerTickAnimator::startTimeTickAnimator("BULLET_ENEMYTickAnimation", AnimatorHolder::triggerBullets);
    ...
    //big plane
    TimerTickAnimator::startTimeTickAnimator("DAIHIRYU_LARGEIntroTickAnimation", triggerBigGreenIntroTickAnimators );
    TimerTickAnimator::startTimeTickAnimator("DAIHIRYU_LARGEIntroTickAnimation", triggerBigGreyIntroTickAnimators );
    //WARNING DONT USE THE ABOVE 2 ANIMATORS
    //TimerTickAnimator::startTimeTickAnimator("DAIHIRYU_LARGEStayStillTickAnimation", triggerBigGreenStayStillTickAnimators );
    //TimerTickAnimator::startTimeTickAnimator("DAIHIRYU_LARGEOutroTickAnimation", triggerBigGreenOutroTickAnimators );
...
namespace {
void loadAnimatorsFromCppData(void)
{
    for (size_t i = 0; i < CppGameData::kAnimatorCount; ++i) {
        const CppGameData::AnimatorInitData& data = CppGameData::kAnimators[i];
        int repetition = data.repetition;
        std::string id = data.id;
        std::string animationId = data.animationId;
        std::string spriteId = data.spriteId;
        int st = data.spriteType;
        SpriteType spriteType = SpriteType(st);
        assert( st < SPRITE_TYPE_SIZE );
        
        std::string id2,spriteId2,animationId2;
        for(int j = 0; j < repetition; ++j){
            id2 = id + std::to_string(j);
            spriteId2 = spriteId + std::to_string(j);
...
void    AnimatorHolder::Load (const std::string& cataloge){
    (void)cataloge;
    loadAnimatorsFromCppData();
}

void AnimatorHolder::LoadFromCppData(void){
    loadAnimatorsFromCppData();
}
...
void AnimatorHolder::createUpdateScoreAnimator(SDL_Rect rect,int points){
    ...
    AnimationFilm* scoreFilm = AnimationFilmHolder::Get()->GetFilm("SCORE");
